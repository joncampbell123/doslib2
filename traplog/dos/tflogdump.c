...
int main(int argc,char **argv) {
	...
	while ((rd=read(fd,buffer,4)) == 4) {
		...
		if (rec->r_recid == 0x8086) {
			...
			printf("       AX=%04x%s BX=%04x%s CX=%04x%s DX=%04x%s SI=%04x%s DI=%04x%s BP=%04x%s SP=%04x%s\n",
			...
			printf("       DS=%04x%s SS=%04x%s ES=%04x%s\n",
