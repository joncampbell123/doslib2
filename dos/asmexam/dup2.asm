...
mkdir_ok:	mov	[filehandle],ax	; save the file handle returned by DOS
		...
		mov	ah,0x46		; AH=0x46 duplicate handle into handle #2
		mov	bx,[filehandle]	; BX=handle to dup2
		mov	cx,2		; CX=handle to replace with duplicate (replace STDERR)
		int	21h
		mov	dx,str_fail
		jc	exit_err	; CF=1 if error

		mov	ah,0x3E		; close original handle
		mov	bx,[filehandle]
		int	21h
		mov	word [filehandle],2 ; make it our file handle
