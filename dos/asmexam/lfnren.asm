...
; LFNREN.COM
;
; Rename a file using Windows 95 long filename.
; NOTE: Long filenames can contain spaces, so for this program you use a double-quote
;       character to separate the two names.
;--------------------------------------------------------------------------------------
...
l2:		lodsb
		cmp	al,13
		jz	end_not_enuf
		cmp	al,'"'		; we're looking for " as delimiter
		jnz	l2
...
do_rename:	mov	ax,0x7156	; long filename rename
		mov	dx,[srcname_p]	; DS:DX = source name
		mov	di,[dstname_p]	; ES:DI = new name
		int	21h
