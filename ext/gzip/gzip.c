...
/* If nonzero then exit with status 2, rather than with the usual
...
int main (int argc, char **argv, char **envp)
{
    ...
    if (4 < proglen && strcasecmp (program_name + proglen - 4, ".exe") == 0)
     ...
     */
    if (strncasecmp (program_name, "un",  2) == 0     /* ungzip, uncompress */
        || strncasecmp (program_name, "gun", 3) == 0) /* gunzip */
        decompress = 1;
    else if (strcasecmp (program_name + 1, "cat") == 0     /* zcat, pcat, gcat */
             || strcasecmp (program_name, "gzcat") == 0)   /* gzcat */
...
#ifdef WINFCON_ENABLE
	unsigned int tmpmax = 256;
	char *tmp = malloc(tmpmax);
	unsigned int tmpi=0;
	int c;
	...
	verbose = 1;
        ...
        WARN ((stderr, "%s: %s GUI prompt follows, this is non-standard port!\n",
               program_name,"non-standard"));

	printf("Enter filename to process or CTRL+C to quit:\n");
	do {
		c = getch();
		if (c >= 32) {
			if ((tmpi+1U) < tmpmax) {
	...
	printf("Compressing or Decompressing [cD]? ");
	do {
		c = tolower(getch());
		if (c == 'c') {
			decompress = 0;
	...
	to_stdout = 0;
	printf("\n%sing %s\n",decompress ? "decompress" : "compress",tmp);
    ...
    char *iname;
{
    ...
    /* Windows 3.1 file manager hack: For a file named "GZIP" it will pass in the full path
     * and "GZIP." */
    {
	    int x = strlen(iname);
	    if (x > 0 && iname[x-1] == '.') iname[x-1] = 0;
    }
...
/* ========================================================================
 * Create the output file. Return 0 or 1.
...
#endif
    int fd;
    int open_flags = (O_RDONLY | (ascii && !decompress ? 0 : O_BINARY));
...
/* ========================================================================
 * Generate ofname given ifname. Return 0, or 2 if file must be skipped.
 * Sets save_orig_name to true if the file name has been truncated.
 */
 ...
 * Return 1 if the file must be skipped.
 */
